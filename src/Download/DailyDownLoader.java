...
public class DailyDownLoader {
	...
	public void processDownLoadList(){
		...
		try(BufferedReader br = new BufferedReader(new InputStreamReader(new FileInputStream("IKanDou.txt")));
			BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("IKanDouSort.txt"),Charset.forName("UTF-8")))){
			String s = null;
	...
	public void dailyDL(final int num) throws IOException{
		try(BufferedReader br = new BufferedReader(new InputStreamReader(new FileInputStream("IKanDouSort.txt"),Charset.forName("UTF-8")))){
